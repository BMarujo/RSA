...
for i in $(seq 1 $RUN_COUNT); do
  ...
  printf "%-3s %-5s %-4s %-4s %-5s %-8s %-5s %-7s %-5s %-5s %-5s %-8s %-8s %-6s %-9s %-4s %-4s\n" "$i" "$TRC" "$WRN" "$COL" "$LANEF" "$HOSTLESS" "$START" "$MERGING" "$CLEAN" "$AFTER" "$TOTAL" "$LEADONLY" "$LOSTAUTH" "$PMLOCK" "$TTCREJECT" "$SPD0" "$T018"
done
